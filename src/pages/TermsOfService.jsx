import React from "react";
import SEO from '../components/SEO';

const TermsOfService = () => (
  <div className="max-w-2xl mx-auto py-16 px-4 md:px-0">
    <SEO title="Terms of Service" description="Terms of Service for Lotus Collective website." />
    <h1 className="text-3xl md:text-4xl font-sans tracking-widest mb-8 text-gray-100" style={{ letterSpacing: '0.15em', fontWeight: 400 }}>
      Terms of Service
    </h1>
    <p className="text-base md:text-lg text-gray-200 mb-6" style={{ fontWeight: 300, letterSpacing: '0.05em' }}>
      The content on this site is for informational and promotional purposes only for Lotus Collective. Unauthorized use of images, music, or text is not permitted. By browsing this site, you agree to these terms.
    </p>
    <p className="text-base md:text-lg text-gray-400" style={{ fontWeight: 300, letterSpacing: '0.05em' }}>
      For questions about these terms, please contact us through our official channels.
    </p>
  </div>
);

export default TermsOfService;
